...
const contenu20Questions = `❓ QUESTION 1 : Comment puis-je être sauvé ?
...
❓ QUESTION 3 : Peut-on perdre son salut ?

RÉPONSE :
Oui, la Bible nous avertit que le salut peut être perdu si nous abandonnons la foi ou retournons délibérément au péché.

"Car il est impossible que ceux qui ont été une fois éclairés, qui ont goûté le don céleste, qui ont eu part au Saint-Esprit, qui ont goûté la bonne parole de Dieu et les puissances du siècle à venir, et qui sont tombés, soient encore renouvelés et amenés à la repentance." (Hébreux 6:4-6)
...
C'est pourquoi nous devons persévérer dans la foi :
...
❓ QUESTION 11 : Qu'est-ce que la repentance ?
...
❓ QUESTION 12 : Pourquoi dois-je me faire baptiser ?
...
❓ QUESTION 13 : Comment puis-je résister aux tentations ?
...
❓ QUESTION 14 : Pourquoi Dieu permet-il la souffrance ?
...
❓ QUESTION 15 : Qu'est-ce que le Saint-Esprit ?
...
❓ QUESTION 16 : Comment puis-je être sûr que Dieu existe ?
...
❓ QUESTION 17 : Pourquoi y a-t-il plusieurs religions ?
...
❓ QUESTION 18 : Que signifie "naître de nouveau" ?
...
❓ QUESTION 19 : Comment puis-je grandir dans ma foi ?
...
❓ QUESTION 20 : Comment puis-je partager ma foi avec les autres ?
...
async function updateTo20Questions() {
  try {
    ...
    const result = await Ressource.updateOne(
      ...
      {
        $set: {
          contenu: contenu20Questions,
          description: 'Réponses bibliques aux 20 questions les plus posées lors de l\'évangélisation. Chaque question est accompagnée de versets bibliques.'
        }
      }
    );

    if (result.modifiedCount > 0) {
      console.log('✅ Ressource mise à jour avec 20 questions!\n');
      console.log('📋 Liste des 20 questions:');
      const questions = [
        'Comment puis-je être sauvé ?',
        'Dois-je faire de bonnes œuvres pour être sauvé ?',
        'Peut-on perdre son salut ? (OUI - avec versets)',
    ...
    console.log('\n═══════════════════════════════════════');
    console.log('   20 Questions - Position: Le salut');
    console.log('   PEUT être perdu (Hébreux 6:4-6)');
    console.log('═══════════════════════════════════════');
