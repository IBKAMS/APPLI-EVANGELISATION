...
async function addUser() {
  try {
    ...
    const existingUser = await User.findOne({ telephone: '0778463449' });
    if (existingUser) {
      if (existingUser.role !== 'evangeliste') {
        await User.updateOne({ telephone: '0778463449' }, { $set: { role: 'evangeliste' } });
        console.log('✅ Rôle mis à jour vers evangeliste');
      } else {
        console.log('⚠️ Utilisateur KOUADIO ANGE existe déjà');
      }
    ...
    console.log('   Nom: KOUADIO');
    console.log('   Prénom: ANGE');
    console.log('   Téléphone: 0778463449');
